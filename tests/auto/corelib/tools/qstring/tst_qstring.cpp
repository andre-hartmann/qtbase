...
//    TEST_SHORT("1.2.3", 1, 1.2); // invalid: two decimals dots
...
void tst_QString::toFloat()
{
    ...
    int endpos;
    QCOMPARE(a.toFloat(&ok, &endpos), 0.000000000931322574615478515625f);
    QVERIFY(ok);
...
//    QTest::newRow("ok12") << QString("100,") << 100.0 << true << 3; // behavior changed with entptr
    ...
    QTest::newRow("wrong04") << QString("100.1.") << 0.0 << false << 0;

//    QTest::newRow("wrong05") << QString("100e") << 100.0 << true << 3; // true with endptr, false otherwise
//    QTest::newRow("wrong06") << QString("100e1e") << 0.0 << false << 0;
//    QTest::newRow("wrong07") << QString("123erf") << 0.0 << false << 3; // ???? 123e is valid?

    QTest::newRow( "wrong08" ) << QString("abc") << 0.0 << false << 0;
    QTest::newRow( "wrong09" ) << QString() << 0.0 << false << 0;
    QTest::newRow( "wrong10" ) << QString("") << 0.0 << false << 0;
}
